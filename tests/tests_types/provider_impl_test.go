...
func TestProviderImpl_NameAndVersion(t *testing.T) {
    ...
    if p.Name() != "openai" { t.Fatalf("Name() = %q, want 'openai'", p.Name()) }
    if p.Version() != "v1" { t.Fatalf("Version() = %q, want 'v1'", p.Version()) }
}
...
func TestProviderImpl_IsAvailable(t *testing.T) {
    p := &typesx.ProviderImpl{}
    if p.IsAvailable() { t.Fatalf("IsAvailable() = true, want false when VAPI nil") }

    p = &typesx.ProviderImpl{VAPI: &m.APIConfigMock{Available: true}}
    if !p.IsAvailable() { t.Fatalf("IsAvailable() = false, want true") }
}
...
func TestProviderImpl_GetCapabilities_DirectAPI(t *testing.T) {
    ...
    if caps == nil { t.Fatalf("GetCapabilities() = nil, want non-nil") }
    if len(caps.Models) != 2 { t.Fatalf("models len = %d, want 2", len(caps.Models)) }
    if caps.MaxTokens <= 0 { t.Fatalf("MaxTokens should be > 0") }
}
...
func TestProviderImpl_GetCapabilities_LazyInitFromConfig(t *testing.T) {
    ...
    caps := p.GetCapabilities()
    if caps == nil { t.Fatalf("GetCapabilities() = nil, want non-nil via lazy init") }
