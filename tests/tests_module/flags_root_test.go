...
func findCmd(root *cobra.Command, name string) *cobra.Command {
    if root.Use == name { return root }
    for _, c := range root.Commands() {
        if got := findCmd(c, name); got != nil { return got }
    }
    return nil
}
...
func TestFlagsFromRoot_AskGenerateChatStart(t *testing.T) {
    m := mod.RegX()
    root := m.Command()
    if root == nil { t.Fatalf("root command is nil") }

    // ask
    ask := findCmd(root, "ask")
    if ask == nil { t.Fatalf("ask command not found from root") }
    ...
    if gen == nil { t.Fatalf("generate command not found from root") }
    ...
    if chat == nil { t.Fatalf("chat command not found from root") }
    ...
    if start == nil { t.Fatalf("server start command not found from root") }
