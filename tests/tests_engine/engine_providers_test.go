...
func TestEngine_GetProviders_FromConfig(t *testing.T) {
    ...
    e := eng.NewEngine(cfg)
    if e == nil { t.Fatalf("NewEngine returned nil") }

    provs := e.GetProviders()
    if len(provs) == 0 { t.Fatalf("expected at least one provider") }

    names := map[string]bool{}
    for _, p := range provs { names[p.Name()] = true }

    // Expected: all configured + ollama
    for _, want := range []string{"openai", "claude", "gemini", "deepseek", "ollama"} {
        if !names[want] { t.Fatalf("provider %q not found in engine", want) }
    }
}
...
func TestEngine_History_SaveAndGet(t *testing.T) {
    ...
    h := e.GetHistory()
    if len(h) != 1 { t.Fatalf("history len = %d, want 1", len(h)) }
