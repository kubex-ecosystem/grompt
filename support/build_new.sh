...
: "${DIST_DIR:=dist}"
: "${ENABLE_UPX:=${_WILL_UPX_PACK_BINARY:-1}}"  # 1 to compress with upx
...
# If BUILDINFO_PATH='main', injects -X <path>.Version/.Commit/.BuildDate with git info.
: "${BUILDINFO_PATH:=main}"
...
build_all() {
  ...
  if [[ -f Makefile ]]; then
    log info "Makefile detected. Running target '${MAKE_TARGET:-build-dev}...'..."
    # shellcheck disable=SC1007
    if MAKEFLAGS= FORCE=y make "${MAKE_TARGET:-build-dev}"; then
      log success "Build via Makefile completed."
      return 0
    else
      log error "Target '${MAKE_TARGET:-build-dev}' failed. Fallback to 'go build'."
    fi
  fi
